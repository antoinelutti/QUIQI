...
## ADDITIONAL NOTES
•	Regional grey matter analysis can in principle be conducted using the provided script (see getROIpairs function for an example). However, ReML computation starts with the selection of the appropriate voxels, using an F-contrast that includes all the regressors of the design matrix. With the data and analysis of interest here, some regions yield no significant voxels and the ReML computation fails. Regional analysis results were therefore not provided. If such analysis was desirable, one solution may be to edit the p value for this test (UFp in spm_est_non_sphericity.m).
